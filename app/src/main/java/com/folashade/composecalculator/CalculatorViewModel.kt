    ...
    private fun performCalculation() {
        ...
        if (number1 != null && number2 != null){
            val result = when(state.operation){
                ...
                is CalculatorOperation.Exponentiate -> Math.pow(number1, number2)
                null -> return
            }
            state = state.copy(
    ...
    private fun enterOperation(operation: CalculatorOperation) {
        if (state.number1.isNotBlank()) {
            state = when (operation) {
                // ... other operations ...
                CalculatorOperation.Exponentiate -> state.copy(operation = operation, number2 = "")
                else -> state.copy(operation = operation)
            }
        }
    }
